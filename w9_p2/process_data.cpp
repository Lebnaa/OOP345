...
namespace sdds
{
	...
	int ProcessData::operator()(const std::string& target_file, double& avg, double& var)
	{
		...
		//creating threads to calculate 

		vector<thread> threads; 
		for (int i = 0; i < num_threads; i++)
		{
			threads.push_back(thread(computeAvgFactorThreaded, data + p_indices[i], p_indices[i + 1] - p_indices[i], ref(averages[i]))); 
		}
		...
		for (int i = 0; i < num_threads; i++)
		{
			threads.push_back(thread(computeVarFactorThreaded, data + p_indices[i], p_indices[i + 1] - p_indices[i], ref(variances[i])));
		}
		...
		if (!file)
		{
			throw string("File " + target_file + "cannot be opened"); 
		}
